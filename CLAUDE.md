...
| Medya tür grupları | `config/media.php` > `types` (image/video/document) **tek kaynaktır**; `accepts` ondan türetilir. Okuyan taraf `App\Support\MediaType`: HTML `accept` özniteliği, kütüphane tür filtresi ve sunucudaki yükleme kontrolü aynı listeyi kullanır — yeni bir uzantı yalnızca config'e eklenir. **`<x-admin::form.image>` bir GÖRSEL alanıdır**: dosya seçicisi yalnızca görsel uzantısı gösterir, `data-media-accept="image"` taşır ve kütüphane seçicisini `?accept=image` ile açar (video/döküman hiç listelenmez, tür filtresi de basılmaz). Sürükle-bırak HTML `accept`'ini atladığı için kontrol sunucuda da var: `accept` parametresi `MediaService::guard()`'a gider. Aynı kalıp `<x-admin::form.video>`'da `video` ile kurulu. Medya kütüphanesi sayfası kısıtsızdır — görsel, video ve döküman (pdf/ofis/arşiv, 32 MB) yüklenir; görsel olmayan dosya önizlemede tür ikonu + "Yeni Sekmede Aç" ile açılır. Editörde iki ayrı buton: görsel ekler (`medialibrary`) ve döküman bağlantısı basar (`filelibrary`) |
...
| Blog taksonomisi (ön yüz) | Kategori ve etiketin ziyaretçi tarafı: `/blog/kategori/{slug}` ve `/blog/etiket/{slug}` — `BlogController::category()/tag()` + `BlogService::listing(category:, tag:)`, **üçü de aynı view** (`pages/blog/index.blade.php`; başlık/meta/kırılım seçili taksonomiden türer). Kategori metası kendi `HasSeo` kaydından gelir, etiketin SEO kaydı yok — adından türetilir. `BlogService::taxonomy()` detay sayfasının kenar çubuğuna **yayında yazısı olan** kategori/etiketleri verir (`whereHas`+`withCount`; `having` kullanılmaz — `ONLY_FULL_GROUP_BY` kıpar). Boş taksonomi hiç basılmaz, ama slug eşleşen boş kategori/etiket **404 değil** boş durumla açılır. `BlogCategory` artık `LinksToPublicPage` + `RedirectsOnMove` (slug değişince otomatik 301, `config/redirects.php`) ve menüden seçilebilir (`config/menus.php` > `linkables.blog-category`). `Tag::blogs()` morphedByMany ile blog tarafını verir — etiket sözlüğü modüller arası ortak olduğu için etiket sayfası yalnızca blogları listeler. Kategori + etiket adresleri site haritasında **`blog` kaynağının içinde**: `enabledSources()` kaydedilmiş listeye baktığı için yeni bir kaynak anahtarı mevcut kurulumlarda sessizce KAPALI başlardı. CSS `public/assets/css/pages/blog/taxonomy.css` (`@push('css')`) |
...
| Canonical + robots meta | `<x-site.meta />` her sayfaya **tek** `<link rel="canonical">` basar. Adres `App\Support\Canonical::current()`'tan gelir: `url()->current()` sorgu dizesini tamamen atıyordu (`?page=2` kendini 1. sayfanın kopyası ilan ediyordu), `url()->full()` ise her `utm_*`/`fbclid` için ayrı canonical üretirdi — bu yüzden yalnızca **içeriği değiştiren** parametreler korunur (`CONTENT_PARAMS`, şu an sadece `page`; `page=1` düşürülür). Kaydın SEO alanındaki canonical ve robots değerleri view'dan `@section('canonical')` / `@section('robots')` ile geçilir — **önceden hiç basılmıyorlardı**: paneldeki Canonical URL alanı ve "arama motorlarına kapat" anahtarı ön yüzde ölüydü (noindex yalnızca site haritasından düşürüyordu). `robots` yalnızca varsayılandan (`index,follow`) sapınca basılır, `og:url` canonical'ı izler. **Hizmet × bölge sayfaları kaydın canonical'ını KULLANMAZ** (`@if (! $region)`): hizmetin tek bir `canonical_url`'u var, her bölge onu basarsa tüm bölge sayfaları tek adrese çöker ve yerel SEO stratejisi çöp olur — bölgede sayfa kendine döner. Yeni bir SEO kayıtlı ön yüz sayfası eklerken bu iki section'ı da geç |
| Teklif formu + yüzen CTA | `<x-site.quote-form>` iki adımlı teklif formudur ve **iki yerde birden** basılır: hizmet detayının kenar çubuğu (`:service`/`:region` ile o hizmet seçili gelir) ve `<x-site.floating-cta>` içindeki site geneli açılır pencere (`compact`). Aynı sayfada iki kopya bulunduğu için **alan kimlikleri örneğe özel önekle** basılır ve JS kimliğe değil `[data-quote-widget]` kapsayıcısına bağlanır (`public/assets/js/quote-form.js`, her kopyayı ayrı ayrı başlatır; alanları `name` ile bulur). Formda `@csrf` **vardır**: JS token'ı başlıkla da yollar ama JS devre dışıyken native POST 419 ile ölmesin. CSS/JS `layout/partials/css.blade.php` + `scripts.blade.php`'de **küresel** yüklenir — bileşenden `@push` ile gönderilemez, çünkü açılır pencere layout'un sonunda basılıyor ve `<head>` o sırada çoktan render edilmiş oluyor (sessizce yüklenmez). Yüzen butonlar sağ altta: "Teklif Al" pencereyi açar, WhatsApp `Ayarlar > Firma Bilgileri > WhatsApp numarası`nı kullanır (boşsa `company.phone`, ikisi de boşsa buton hiç basılmaz); bağlantıyı `App\Support\Phone::whatsapp()` üretir — `wa.me` YALNIZCA rakam kabul eder, `+`/boşluk kalırsa boş sohbet açılır. Hizmet detayında kenar çubuğu mobilde içerikten SONRA gelir (`order-2 order-lg-1`) |
| Ön yüz butonları | **Tek sistem: `.ui-btn` + bir varyant** (`public/assets/css/main.css`). `--solid` birincil (koyu dolgu), `--outline` ikincil (çerçeveli); zemin koyu/fotoğrafsa `--light` eklenir, boyut için `--sm`/`--lg`, tam genişlik `--block`, yalnızca ikon taşıyan buton `--icon`. İkon doğrudan `<i>` olarak içine yazılır (hover'da sağa kayar), temannın `.arrow1`/`.arrow2` span'leri KULLANILMAZ. Temannın `.theme-btn*` / `.default-btn` class'ları ölüdür, canlı Blade'lerde hiçbiri kalmadı — yenisi yazılmaz, öyle bir buton görülerse `.ui-btn`'e çevrilir. Kapsayıcıya özgü düzen (teklif sihirbazında satırı doldurma, çerez şeridinde mobilde yarım genişlik) buton class'ına değil **kapsayıcıya** yazılır (`.quote-nav .ui-btn`, `.cookie-banner__actions .ui-btn`) |
| Ön yüz sayfalaması | `resources/views/vendor/pagination/theme.blade.php` — temanın `.theme-pagination` markup'ı, `{{ $paginator->links('vendor.pagination.theme') }}` ile çağrılır. Laravel'in varsayılan paginator view'leri Tailwind olduğu için ön yüzde doğrudan kullanılamaz (bozuk görünür); yeni bir sayfalı ön yüz listesi bu view'i kullanır |
...
| Schema.org (JSON-LD) | `App\Services\Schema\{SchemaGraphBuilder,SchemaLinter,SchemaInspector}` + `App\Support\SchemaContext`. Ön yüzde `<x-site.schema />` her sayfaya bağlı bir `@graph` basar (Organization/ProfessionalService + WebSite + WebPage + BreadcrumbList + sayfa türüne göre Service/BlogPosting/FAQPage). Ayarlar: `setting` grubu `schema` (`setting.schema.update`). Kayıt bazında override: `seo` tablosunda `schema_type`/`schema_json`/`schema_override` + `<x-admin::form.schema>` (Blog/Hizmet/Sayfa formlarında). Doğrulama ekranı `/admin/schema`. Dinamik controller'lar view'e `schemaContext` geçirir; layout onu `:context` ile bileşene verir, yoksa `SchemaContext::fromRoute()` |
...
| Duyuru şeridi + popup | Ayrı modüller (`Announcement`, `Popup`), ortak `HasAudience` (tüm site / anasayfa / seçili sayfa-yazı-hizmet + `starts_at`/`ends_at`). Ön yüz `NoticeResolver` o isteğe uyan en yeni kaydı verir; markup `<x-site.notices />`. Kapatma `localStorage`'da kalıcı. Popup bülten formu açabilir (`collect_email`). |
| Bülten aboneleri | `Subscriber` + `/admin/subscriber` + CSV. Ön yüz `POST /bulten` (honeypot + KVKK onayı, `throttle:newsletter`), ayrılma `GET /bulten/ayril/{token}`. Çift onay yok. Footer + popup `<x-site.subscribe-form />`. |
...
| Hizmet bölgeleri (yerel SEO) | `App\Models\ServiceRegion\ServiceRegion` (sonsuz derinlikli ağaç, il `depth=0` ve id'si plaka kodu) + `Service` ile çoktan-çoğa. Hizmet içeriği **bir kez** yazılır, `{{region}}`/`{{city}}`/`{{district}}` yer tutucuları bölge başına çözülür (`Service::renderFor()`); bölgesiz şemsiye sayfada yer tutucular atılır (`renderGeneric()`). **Adres iç içedir**: `/hizmetler/{hizmet}/{il}/{ilçe}` — route `{region}` parametresi birden çok segment tutar (`where`'de kısıt elle verilir), çözüm `service_regions.slug_path` kolonu üzerinden tek karşılaştırmayla yapılır (`ServiceService::findRegion()`). `path` adların zinciri ("Gaziantep Şahinbey"), `slug_path` adres karşılığı ("gaziantep/sahinbey") — ikisi de türetilmiş, `ServiceRegionService::derive()` yazar ve ad/slug değişince alt ağaç `refreshSubtree()` ile tazelenir; **adres üreten her yer `slug_path` kullanır**, `slug` değil (sitemap, şema, kenar çubuğu, kırık link). Duplicate content'e karşı iki mekanizma: `service_regions.description` ("Bölgeye Özel Metin") detay sayfasında içerikten sonra `.region-note` bloğu olarak basılır — bu, bölge sayfalarının özgün olan tek parçasıdır, basılmazsa alanın varlık sebebi kalmaz; ve meta başlık/açıklamada bölge zincirinden hiçbir ad geçmiyorsa `Service::qualifySeo()` bölge adını başa ekler (yer tutucu kullanılmışsa dokunmaz). Kenar çubuğundaki liste ilçeleri ilin altında gruplar (`ServiceService::regionGroups()` — kök kayıtlar `slug_path`'in ilk parçasından tek sorguda çözülür, ağaç yukarı gezilmez); bağlı olmayan il başlık olarak görünür ama linklenmez, aynı kural kırılımda da geçerli. 8'den fazla bölgede kenar çubuğuna istemci tarafı arama kutusu çıkar (`pages/services/show.js`) — filtre yalnızca görünürlüğü kısar, linkler HTML'de kalır |
| Neler Yaptık (projeler) | `App\Models\Project\Project` + `ProjectCategory` + `App\Services\Project\ProjectService`, `/admin/project` ve `/admin/project-category`. Vaka çalışması kaydı: künye (müşteri/sektör/tarih/süre/yayındaki adres), `technologies` ve `results` **JSON** kolonları (kendi başlarına sorgulanmadıkları ve sırası kullanıcının verdiği sıra olduğu için ayrı tablo açılmadı — revizyon diff'ine de alan olarak kendiliğinden girer), galeri (`form.image multiple`), video (gömülü adres **ya da** kütüphaneden mp4, ikisi birlikte değil), SSS, SEO, etiket, revizyon, toplu işlem. Projeler hizmetlere çoktan-çoğa bağlanır (`project_service`) — hizmet sayfasında "bu hizmette yaptığımız işler" bloğu kurulabilsin diye. Müşteri yorumu `testimonial_id` ile bağlanır. Kırık link taraması ve global arama kaynakları arasında. **Ön yüz**: `/projeler` (liste), `/projeler/kategori/{slug}` (kategori), `/projeler/{slug}` (detay) — `App\Http\Controllers\Project\ProjectController` + `ProjectService::listing()/related()/findCategoryBySlug()`, view'ler `resources/views/pages/projects/`. Üç route da `module.active:project,404` altında: Modül Yönetimi'nden kapatılınca ziyaretçiye 404 döner. `publicUrl()` yayın durumuna EK OLARAK modül durumuna da bakar (kapalıyken null döner, menü öğeleri ve kartlar kendiliğinden düşer); `indexNowUrl()` ikisine de bakmaz. Kart markup'ı **tek partial** (`pages/projects/partials/card.blade.php`) — liste, "Benzer İşler" ve hizmet detayındaki "Bu Hizmette Yaptığımız İşler" bloğu aynı dosyayı kullanır. Schema.org'da `CreativeWork` düğümü basılır (müşteri `about` altında Organization, kategori `genre`); site haritasında `projects` kaynağı liste + kategori + proje adreslerini **birlikte** üretir (liste adresi bilinçli olarak `static_routes`'ta değil — modül kapanınca hepsi tek yerden düşsün). Kategori slug'ı değişince de 301 üretilir, çünkü kategori sayfaları indekslenebilir |
