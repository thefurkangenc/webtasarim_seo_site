...
<form id="reference-form" data-id="{{ $reference?->id }}" enctype="multipart/form-data">
    ...
    <x-admin::form.image name="logo_media_id" label="Firma Logosu" required
        :media="$reference?->getFirstMedia('logo')" />

    <x-admin::form.input name="name" label="Firma Adı" required :value="$reference?->name"
        placeholder="Örn. BrightEdge Media" />

    <x-admin::form.input name="url" label="Bağlantı" :value="$reference?->url"
