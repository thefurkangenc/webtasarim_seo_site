<form id="setting-form" action="{{ route('admin.setting.company.update') }}" method="POST">
    ...
    <x-admin::form.textarea name="address" help="company.address" label="Adres" :value="$values['address'] ?? null" placeholder="Açık adres" />

    <x-admin::form.textarea name="short_description" help="company.short_description" label="Kısa Açıklama" :value="$values['short_description'] ?? null"
