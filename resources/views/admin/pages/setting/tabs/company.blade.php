<form id="setting-form" action="{{ route('admin.setting.company.update') }}" method="POST">
    ...
    <x-admin::form.image name="logo_media_id" label="Firma logosu"
        :media="$media['logo_media_id'] ?? null"
        hint="Üst menü ve Google arama önizlemesinde kullanılır." />

    <x-admin::form.input name="name" label="Firma adı" required :value="$values['name'] ?? null"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px] mb-[20px] md:mb-[25px]">
        <x-admin::form.input name="phone" label="Telefon" :value="$values['phone'] ?? null"
            placeholder="Örn. 0212 000 00 00" wrapper="" />

        <x-admin::form.input name="fax" label="Fax" :value="$values['fax'] ?? null"
            placeholder="Örn. 0212 000 00 01" wrapper="" />
    </div>

    <x-admin::form.input name="email" label="E-posta" type="email" :value="$values['email'] ?? null"
        placeholder="Örn. info@ornek.com" />

    <x-admin::form.textarea name="address" label="Adres" :value="$values['address'] ?? null"
