    ...
    <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        ...
        <div class="trezo-card-content">
            <form id="hero-form" action="{{ route('admin.hero.update') }}" method="POST">
                ...
                <x-admin::form.input name="badge" label="Üst Etiket" :value="$hero->badge"
                    placeholder="Örn. Dijital Ajans" />

                <x-admin::form.input name="title" label="Başlık" :value="$hero->title"
                    placeholder="Örn. Markanızı dijitalde büyütüyoruz" />

                <x-admin::form.textarea name="description" label="Açıklama" :value="$hero->description"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px] mb-[20px] md:mb-[25px]">
                    <x-admin::form.input name="button_text" label="Buton Yazısı" :value="$hero->button_text"
                        placeholder="Örn. Teklif Alın" wrapper="" />

                    <x-admin::form.input name="button_url" label="Buton Bağlantısı" :value="$hero->button_url"
