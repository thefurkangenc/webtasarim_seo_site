...
<form id="redirect-form" data-id="{{ $redirect?->id }}">
    ...
    <x-admin::form.input name="from_path" label="Kaynak Adres" required
        :value="$redirect?->from_path ?? request('from')"
        placeholder="Örn. /eski-sayfa" />

    <x-admin::form.select name="match_type" label="Eşleşme Tipi" required
        :options="$matchTypes"
    ...
    <x-admin::form.input name="to_url" label="Hedef"
        :value="$redirect?->to_url"
        placeholder="/yeni-sayfa ya da https://..." />

    <x-admin::form.select name="status_code" label="Durum Kodu" required
        :options="$statusCodes"
    ...
    <x-admin::form.textarea name="notes" label="Not (opsiyonel)" rows="2" :value="$redirect?->notes"
        placeholder="Bu yönlendirme neden var?" class="h-[70px]" />

    <x-admin::form.switch name="is_active" label="Aktif" :checked="$redirect?->is_active ?? true" />
