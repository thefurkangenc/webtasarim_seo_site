...
/*
 * Bölge kısayolları. Choices.js örneği core/select.js tarafından select
 * üzerinde saklanır — native <option>'a yazmak arayüzü tazelemez, bu yüzden
 * seçim Choices API'si ile yapılır (setChoiceByValue / removeActiveItems
 * option.selected'ı da günceller, FormData doğru gider).
 */
const regionSelect = document.getElementById('service_regions');
const selectAll = document.getElementById('service-regions-all');

selectAll?.addEventListener('click', () => {
    // Yalnızca iller (kök bölgeler); alt bölgeler dokunulmadan kalır.
    const cityIds = JSON.parse(selectAll.dataset.cityIds || '[]').map(String);

    if (regionSelect?.choicesInstance) {
        regionSelect.choicesInstance.setChoiceByValue(cityIds);

        return;
    }

    // Choices henüz kurulmadıysa native select'e yaz.
    cityIds.forEach((id) => {
        const option = regionSelect?.querySelector(`option[value="${id}"]`);
...
document.getElementById('service-regions-clear')?.addEventListener('click', () => {
    if (regionSelect?.choicesInstance) {
        regionSelect.choicesInstance.removeActiveItems();
 ...
 * üretim kuyrukta devam eder — sayfa başlığının altında beliren kart
 * ilerlemeyi (ve varsa hatayı) gösterir (bkz. core/ai-progress.js).
 */
const progress = new AiProgress(form, `service:${form.dataset.id ?? 'new'}`, applyOutput);

document.getElementById('service-ai')?.addEventListener('click', async () => {
    const result = await aiGenerator.open('service.content', {
        defaults: {
